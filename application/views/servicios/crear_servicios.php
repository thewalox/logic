	...
	<script>
		$(document).ready(function(){
			...
			$('#aceptar').on('click',function(){
			    ...
			    $.ajax({
			    	type:"POST",
			    	url:"<?php echo base_url('servicios/crear_servicio'); ?>",
			    	data:{
			    		'codigo'		: 	codigo,
			    	...
			    	success:function(data){
						...
						if (json.mensaje == 2) {
							html += "<div class='alert alert-success' role='alert'>Servicio creado Exitosamente!!!!!</div>";
							$("#codigo").val("");
							$("#descripcion").val("");
						}else if(json.mensaje == 3){
								html += "<div class='alert alert-danger' role='alert'>Ah ocurrido un error al intentar crear este servicio. Por favor revise la informacion o comuniquese con el administrador del sistema</div>";
						}else{
