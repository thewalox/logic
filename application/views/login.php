		...
		<form action="" method="post" name="Login_Form" class="form-signin">       
			...
			<div class="input-group" id="emp">
				...
				<select class="form-control" name="empresa" id="empresa">
					...
					<option value="PJ">Plasticos Jaguar SAS</option>
					<option value="IP">Industrias Plasticas del Caribe</option>
				</select>
			</div>
			<div id="content"></div>
...
<script>
		$(document).ready(function(){
			...
			$('#login').on('click',function(){
		    	...
		    	if (emp == 0){
					$( "div[id='emp']" ).addClass( "form-group has-error" );
		            return false;
		    	}else{
		    		$( "div[id='emp']" ).removeClass( "has-error" );
		    	}

		    	encrypt = calcMD5(pass);
				
				$.post("<?php echo site_url('login/validar_usuario'); ?>", 
					{usuario: user, clave: encrypt, empresa: emp}, function(result){
						if (result == "ok") {
